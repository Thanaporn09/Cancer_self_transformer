...
class Mod_position(BaseModule):
    ...
    def forward(self, x, hw_shape, mode='bilinear'):
        ...
        ##### Modification #####
        b, hw, em_bed = x.shape
        pos_embed = x.reshape(b, int(self.pos_shape), int(self.pos_shape), self.pos_dim).permute(0, 3, 1, 2).contiguous()
        pos_embed = self.max_pool(pos_embed)
...
class ConvolutionalPyramidVisionTransformer(BaseModule):
    """
    Args:
    ...
    def __init__(self,
        ...
        for i, num_layer in enumerate(num_layers):
            ...
            if use_abs_pos_embed:
                ...
                pos_shape = (pretrain_img_size[0]/self.reduce_ratio) 
...
@BACKBONES.register_module()
class ConvolutionalPyramidVisionTransformerV2(ConvolutionalPyramidVisionTransformer):
    """Implementation of `PVTv2: Improved Baselines with Pyramid Vision
    Transformer <https://arxiv.org/pdf/2106.13797.pdf>`_."""

    def __init__(self, **kwargs):
        super(ConvolutionalPyramidVisionTransformerV2, self).__init__(
            patch_sizes=[7, 3, 3, 3],
            paddings=[3, 1, 1, 1],
