...
class PatchEmbed_Mod(BaseModule):
    ...
    def __init__(
        ...
        super(PatchEmbed_Mod, self).__init__(init_cfg=init_cfg)
        ...
        ##### Multi-kernel Conv Patch Embedding #####
...
#        #### Multi kernel patch embedding #####
        pat_embed1 = self.conv3(x)      
        pat_embed2 = self.conv5(x)
        pat_embed = pat_embed1 + pat_embed2

        out_size = (pat_embed.shape[2], pat_embed.shape[3])
        #### Global Average Pooling ####
        pat_embed = pat_embed.flatten(2).transpose(1, 2)
        if self.norm is not None:
            pat_embed = self.norm(pat_embed)
            
        return pat_embed , out_size
